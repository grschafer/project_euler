...
# greatest sum --> greatest product (unless 0 is included)
# from looking at it: starting at (15,3) going NE: 87*97*94*89 = 70,600,674
...
def gen_east(arr, row):
    for i in range(arr.shape[0]):
        yield arr[row,i]
def gen_south(arr, col):
    for i in range(arr.shape[1]):
        yield arr[i,col]
def gen_southeast(arr, row, col):
    while row < arr.shape[0] and col < arr.shape[1]:
        yield arr[row,col]
        row += 1
        col += 1
def gen_southwest(arr, row, col):
